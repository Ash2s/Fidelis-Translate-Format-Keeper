...
def test_convert_chinese_dates():
    ...
    assert service.convert_chinese_dates("2025.10.13") == "October 13, 2025"
    # Numeric date YYYY-MM-DD
    assert service.convert_chinese_dates("2025-08-28") == "August 28, 2025"


def test_double_date_cleanup():
    """Double date patterns should be cleaned to single date."""
    result = TranslatorService._clean_mechanical_errors(
        "Review Date: October 13, 2025 2025.10.13"
    )
    assert "October 13, 2025" in result
    assert "2025.10.13" not in result

    result = TranslatorService._clean_mechanical_errors(
        "Document publication date: August 28, 2025 2025-08-28"
    )
    assert "August 28, 2025" in result
    assert "2025-08-28" not in result


def test_cn_punctuation_cleanup():
    """Chinese punctuation should be converted to English in mechanical cleanup."""
    result = TranslatorService._clean_mechanical_errors(
        "邮箱：test@example.com"
    )
    ...
    result = TranslatorService._clean_mechanical_errors(
        "引号，句号。分号；感叹号！问号？括号（）书名《》"
    )
    assert "，" not in result and "。" not in result and "；" not in result
    assert "," in result and "." in result and ";" in result


def test_fix_cn_labels_strips_ai_meta():
    """fix_cn_labels should strip known AI metadata patterns."""
    result = TranslatorService.fix_cn_labels("【英文材料无需翻译】")
    assert "No Translation Required" in result
    assert "无需翻译" not in result
